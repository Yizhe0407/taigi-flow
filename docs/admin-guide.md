...
| 頁面 | 路徑 | 用途 |
|---|---|---|
| 人格 | `/agents` | 建立、編輯、啟用 Agent 人格；各 Role 的 RAG 知識庫也在此管理 |
...
1. 到 `人格` 建立或選擇要使用的 Agent。
2. 編輯 Role 的系統提示詞、工具清單與 RAG 設定。
3. 如果該 Role 需要文件知識，在 Role 編輯頁的「知識庫」tab 上傳文件並等待 ingest 完成。
...
> **注意**：TTS 語音模型、語速與音調目前由環境變數控制（`PIPER_MODEL_PATH`、`PIPER_SPEAKER_ID`），不從 Role 設定讀取。`voiceConfig` 欄位已保留供未來使用，現階段修改不會生效。

RAG 的 collection ID 會使用 Role ID。建立新 Role 時還沒有 ID，所以 RAG 文件上傳要在 Role 建立後進行。

## RAG

每個 Role 對應一個 RAG 知識庫，collection ID 等於 Role ID。RAG 管理整合在 Role 編輯頁（`/agents/:id`）的「知識庫」tab，沒有獨立的知識庫列表頁。

### RAG 設定

在 Role 編輯頁的「知識庫」tab 上方會顯示：
...
如果 RAG 停用，即使已上傳文件，正式對話也不會使用這些內容。要啟用請切換到「基本」tab 開啟 RAG 設定後儲存。
...
- Role 編輯頁「基本」tab 是否已啟用 RAG。
- 「知識庫」tab 的文件是否已 ingest 完成。
- 「知識庫」tab 的「檢索測試」是否能命中內容。
- threshold 是否設太高。
...
- 該 Role 的 system prompt 正確（TTS 速度與 pitch 由 env 控制，非 Role 設定）。
- 若需要 RAG，該 Role 編輯頁「知識庫」tab 的文件已 ingest 完成，且 RAG 在「基本」tab 已啟用。
- 「知識庫」tab 的「檢索測試」能查到預期內容。
- `發音字典` 已包含常見專有名詞。
- `即時監控` 顯示串流連線中。
