...
```bash
# 啟動基礎設施（LiveKit、Ollama、Qwen3-ASR、Piper TTS）
...
> **Qwen3-ASR 首次啟動需要較長時間載入模型**（3.8GB）。請確認 Docker Desktop 記憶體至少設為 **8GB**（Settings → Resources → Memory）。模型放在 `models/qwen3-asr/checkpoint-20340/`，掛載至容器內 `/model`。
...
- `http://localhost:8090/api/sessions/{id}` — 單一 session 詳情
