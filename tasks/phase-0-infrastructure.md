...
- [ ] **Commit 訊息**：`chore: initial project scaffolding`
...
- [ ] **依賴**：P0-01
- [ ] **輸入**：`docs/plan.md §8.1` 的 compose 範例
- [ ] **動作**：
...
- [ ] **Commit 訊息**：`feat(infra): add docker compose with postgres/redis/livekit`
...
- [ ] **依賴**：P0-01
- [ ] **輸入**：`docs/plan.md §7.1` 的 Monorepo 結構
- [ ] **動作**：
...
- [ ] **驗收**：
  ...
  - 兩頁各自顯示 hello world
  - `pnpm build` 全部編譯通過
- [ ] **Commit 訊息**：`feat(web): scaffold monorepo with playground and admin apps`
...
- [ ] **依賴**：P0-02、P0-03
- [ ] **輸入**：`docs/plan.md §2.2` 的完整 Prisma schema
- [ ] **動作**：
...
- [ ] **Commit 訊息**：`feat(db): add prisma schema with pgvector extension`
...
- [ ] **Commit 訊息**：`feat(worker): scaffold python project with uv`
...
- [ ] **Commit 訊息**：`feat(worker): add sqlalchemy models and repositories`
...
- [ ] **驗收**：
  ...
  - 三個 job 都有執行
- [ ] **Commit 訊息**：`ci: add github actions for web, worker, and prisma checks`
...
- [ ] **Commit 訊息**：`docs: update readme with verified setup steps`
...
- [ ] `docker compose up -d` 全綠
- [ ] 兩個前端 app 能啟動
- [ ] Worker 能連 DB 讀到 seed 資料
- [ ] CI 在 main branch 全綠
- [ ] 同團隊新成員能在 1 小時內跑起開發環境
