...
class HanloBridge:
    ...
    def _timed_convert(self, text: str) -> str:
        """呼叫 pipeline 並記錄 span + metric。"""
        span = get_tracer().start_span("converter.convert")
        span.set_attribute("input.length", len(text))
        t0 = time.perf_counter()
        try:
            return self._convert_fn(text)
        finally:
            metrics.conversion_duration_seconds.observe(time.perf_counter() - t0)
            span.end()

    def convert(self, text: str) -> str:
        """單次同步轉換，回傳 Taibun 羅馬字。"""
        self._ensure_pipeline()
        return self._timed_convert(text)
    ...
    async def stream_convert(self, text_stream: AsyncIterable[str]) -> AsyncIterable[str]:
        ...
        async for token in text_stream:
            ...
            while any(d in buffer for d in _SENTENCE_DELIMITERS):
                ...
                if sentence.strip():
                    # HanloFlow conversion is already sentence-buffered. Running it inline
                    # avoids executor/thread lifecycle issues during local development.
                    yield self._timed_convert(sentence.strip())

        if buffer.strip():
            yield self._timed_convert(buffer.strip())
